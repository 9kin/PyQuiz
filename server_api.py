...
def connect(game_id: int, name:str):
    ...
    if game_id not in host_map or host_map[game_id]["question_cnt"] != -1:
        ...
        key = secrets.token_hex(20)
        host_map[game_id]["peoples"][key] = {"score": 0, "last_active" : datetime.datetime.now(), "name": name, "last": {"ans": -1, "res": False, "add": 0}}
    return {"key": key}
...
def play(game_id: int, key: str):
    global host_map
    if game_id not in host_map or host_map[game_id]["key"] != key or host_map[game_id]["question_cnt"] != -1 or len(host_map[game_id]["peoples"]) == 0:
        raise HTTPException(status_code=404, detail="error")
    else:
...
def next_game(game_id: int, key: str):
    global host_map
    if game_id not in host_map or host_map[game_id]["key"] != key or host_map[game_id]["question_cnt"] < 0:
        raise HTTPException(status_code=404, detail="error")
    else:
        ...
        for key in host_map[game_id]["peoples"]:
            host_map[game_id]["peoples"][key]["score"] += host_map[game_id]["peoples"][key]["last"]["add"]
...
def validate(game_id: int, key: str):
    global host_map
    if game_id not in host_map or host_map[game_id]["question_cnt"] < 0 or key not in host_map[game_id]["peoples"]:
        raise HTTPException(status_code=404, detail="error")
    ...
    score = host_map[game_id]["peoples"][key]["last"]["add"] + host_map[game_id]["peoples"][key]["score"]
    place = 1
    arr = []
    ...
    if arr[0][1] == key:
        ...
        for i in range(1, len(arr)):
            ...
            if arr[i][1] == key:
                break
    return {"valid": host_map[game_id]["peoples"][key]["last"]["res"], "score": score, "place": place}
...
def question_info(game_id: int, key: str):
    ...
    if key not in host_map[game_id]["peoples"]:
        raise HTTPException(status_code=404, detail="error")
    cur = str(host_map[game_id]["question_cnt"])
    time_fished = host_map[game_id]["last_active"] + datetime.timedelta(seconds=int(host_map[game_id]["quiz"][cur]["time"]))
    return {"ans": len(host_map[game_id]["quiz"][cur]["answers"]), "time": time_fished - datetime.datetime.now()}
